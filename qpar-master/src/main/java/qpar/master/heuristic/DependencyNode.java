...
public class DependencyNode {
	...
	public DependencyNode(final Integer variable, final NodeType type) {
	...
	public final void setDepth(final Integer depth) {
		this.depth = depth;
		for (DependencyNode d : this.children) {
			d.setDepth(depth + 1);
		}
	}
	...
	public void addChild(final DependencyNode child) {
	...
	public boolean homogenousSubtree() {
		...
		case 0:
			return true;
		case 1:
			return (this.type == this.children.get(0).type && this.children.get(0).homogenousSubtree());
		case 2:
			return ((this.type == this.children.get(0).type) && this.children.get(0).homogenousSubtree()
					&& (this.type == this.children.get(1).type) && this.children.get(1).homogenousSubtree());
		default:
	...
	public CondensedDependencyNode condense() {
		...
		// IF the whole subtree is made up of same quantifiers return one node
		// with them all
		// included
		...
		case 0:
			...
			assert (false);
			LOGGER.error("Algorithm fail... Contact Author.");
			throw new RuntimeException();
		case 1:
			CondensedDependencyNode n = new CondensedDependencyNode(this.type);
	...
	// ArrayList<CondensedDependencyNode> ret = new
	// ArrayList<CondensedDependencyNode>();
	...
	public ArrayList<DependencyNode> allSubnodes() {
		...
		case 0:
			...
			return n0;
		case 1:
			ArrayList<DependencyNode> n1 = this.children.get(0).allSubnodes();
			n1.add(this);
			return n1;
		case 2:
			ArrayList<DependencyNode> n20 = this.children.get(0).allSubnodes();
			ArrayList<DependencyNode> n21 = this.children.get(1).allSubnodes();
			n20.addAll(n21);
			n20.add(this);
			return n20;
		default:
